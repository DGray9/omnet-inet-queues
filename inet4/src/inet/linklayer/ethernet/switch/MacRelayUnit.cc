...
void MacRelayUnit::handleAndDispatchFrame(Packet *packet)
{
    ...
    if (outputInterfaceId >= 0) {
        ...
        *newPacketProtocolTag = *oldPacketProtocolTag;
        ...
        advancedSend(packet, outputInterfaceId);
    }
    else {
...
void MacRelayUnit::advancedSend(Packet *frame, int outputInterfaceId)
{
    sendDelayed(frame, delayTime, "ifOut");
}
